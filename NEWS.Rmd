...
*  Fixed an error occurring when multiple restrictions are applied to a single 
   variable
*  Revised the `gformula()` function so that it does not produce an error message when 
   one of the bootstrap replicates produces a g-form risk or HR of NA. The bootstrap 
   standard errors and 95% CIs are calculated based on the removing all NA values.
